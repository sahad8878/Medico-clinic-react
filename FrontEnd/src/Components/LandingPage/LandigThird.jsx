import React from "react";

function LandigThird() {
 
      
  return (
    <>
      <div
        className="  bg-[#F5F0E2] flex w-screen items-center justify-center "
      >
        <div className=" text-center py-32  w-[1000px] ">
          <h1 className="font-mono text-4xl font-bold ">
            WE'RE BUILDING A HOME FOR <br /> HEALTHCARE
          </h1>
          <h1 className="mt-10 text-3xl font-serif  ">
            A place where every type of patient can find every type of care. A
            place where they can see comprehensive providers and treatment
            options, with cost and quality info . A place where they can choose
            from a marketplace that is competing to serve them.
          </h1>
        </div>
      </div>
    </>
  );
}

export default LandigThird;
